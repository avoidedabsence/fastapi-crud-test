...
class BuildingOut(BaseModel):
    ...
    organizations: Optional[List["OrganizationOut"]] = Field(
        default=None,
...
class OrganizationOut(BaseModel):
    ...
    activities: Optional[List[ActivityOut]] = Field(
        default=None,
...
class OrganizationIn(BaseModel):
    ...
    phone: list[str] = Field(
        description="Список номеров телефонов организации",
        examples=[["79999999999", "79888888888", "etc..."]],
    )
    ...
    activity_ids: list[int] = Field(
        description="Список ID деятельностей организации",
        examples=[[1, 2]],
    )
...
class BuildingUpdate(BaseModel):
    ...
    addr: Optional[str] = Field(
        default=None,
        description="Адрес здания (optional)",
        examples=["ул. Пушкина д. 1"],
    )
    lat: Optional[float] = Field(
        default=None,
        description="Широта здания (optional)",
        examples=[56.4],
    )
    lon: Optional[float] = Field(
        default=None,
        description="Долгота здания (optional)",
        examples=[32.3],
    )
